...
const LevelUpGolferModal = props => {
  ...
  const applyMedals = (type) => {
    console.log('[LevelUpGolferModal] - applyMedals ' + type);
  ...
  const unapplyMedals = (type) => {
    console.log('[LevelUpGolferModal] - unapplyMedals');
    setMedalsApplied(medalsApplied - 1);
    switch (type) {
      case 'power':
        setPowerMedals(powerMedals - 1);
  ...
  return (
    <Modal
      title="Level Up Your Golfer"
      onOpen={props.onOpen}
      callbackClose={props.callbackClose}
      lobbyModal={true}
    >
      <Modal title='Golfer Upgraded' onOpen={golferUpgradeSuccess} callbackClose={() => golferUpgradeResponse(false)}></Modal>
      <div className="grid gap-5">
        <div className='flex justify-between mx-3 mt-3'>
            <p className='text-dark-indigo text-2xl font-black mb-0'>Level Up Golfer</p>
            <img
              onClick={() => {
                props.callbackClose();
              }}
        ...
        <div className="w-full overflow-hidden" style={{backgroundImage: 'url(' + props.background + ')', backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat', minHeight: '100px'}}>
            <div className='flex justify-between items-center h-full'>
                <Parallelogram className="bg-dark-indigo w-24 h-10 text-center">
                    <Industry className="mt-2 text-white font-bold text-lg">
                        {props.golferDetails.tokenid}
                    </Industry>
                </Parallelogram>
                <img
                    src={getBadgeSvg(collectibleType)}
                    alt={`${playerType} player icon`}
                    className={props.large ? "ml-auto mt-5 mr-5 w-16 h-12" : "ml-auto my-auto mr-3 w-24 h-20"}
                />
            </div>
        </div>
        <LobbyProgressCard
        ...
        {medalsApplied === 0 && insuffcientMedals === false && (
          <ApplyMedalsDeactivated
            className='mx-auto py-1 mb-6 text-grey font-black text-sm'
            >
            Apply Medals
          </ApplyMedalsDeactivated>
        )}
        {insuffcientMedals && (
          <ApplyMedalsError
            className='mx-auto py-1 mb-6 text-error-red font-black text-sm'
          >
            Insufficient Medals
          </ApplyMedalsError>
        )}
        {medalsApplied > 0 && insuffcientMedals === false && (
          <ApplyMedals
            className='mx-auto py-1 mb-6 text-white font-black text-sm'
...
export default connect(mapProps)(LevelUpGolferModal);